...
//let connect_url = "http://192.168.1.247:8899/stl-api";
...
let customer_contact_update = connect_url + '/api/Customer_Contact_Update';
let customer_contact_delete = connect_url + '/api/Customer_Contact_Delete';
//let saletarget_get = connect_url + '/api/saletarget_get';
let customer_owner_create = connect_url + '/api/Customer_Owner_Create';
let customer_owner_delete = connect_url + '/api/Customer_Owner_Delete';
let customer_owner_update = connect_url + '/api/Customer_Owner_Update';
...
let customer_bankaccount_update = connect_url + '/api/Customer_BankAccount_Update';
let customer_bankaccount_delete = connect_url + '/api/Customer_BankAccount_Delete';
let customer_bankaccount_get = connect_url + '/api/Customer_BankAccount_Get';
let emmas_eprovinc_val = "";
let salegrch_eprovinc2_val = "";
let emmasaddr_eprovinc_val = "";
...
let route_get = connect_url + '/api/Customer_Setup_Route_Get';
...
let customer_sale_select2_get = connect_url + '/api/Customer_Sale_Select2_Get';
...
let url_lov_zone_get = connect_url + '/api/Lov_Zone_GET'
let url_lov_route_get = connect_url + '/api/Lov_Route_GET'
let url_lov_subroute_get = connect_url + '/api/Lov_SubRoute_GET'
...
let bank_list = [];
let bank_multilist = [];
let bank_multiselect; //kung 20/10/2022
let lov_zone = [];
...
$.init = function () {
    ...
    //$('#search_samas_sale').select2({
    //    dropdownAutoWidth: true,
    //    width: 'resolve'
    //})

    $('#search_samas_sale').select2({
        dropdownParent: $('#saleinfo'),
        minimumInputLength: 1,
        minimumResultsForSearch: 10,
        dropdownAutoWidth: true,
        delay: 500,
        ajax: {
            url: customer_sale_select2_get,
            dataType: 'json',
            width: 'resolve',
            dropdownAutoWidth: true,
            //width: 'auto',
                ...
                return {
                    results: $.map(data.data, function (item) {
                        return {
                            ...
                            support_supervisor: item.support_supervisor,
                            sale_support1: item.sale_support1,
                            sale_support2: item.sale_support2
                        }
                    })
                };
            },
    ...
    $('#search_samas_sale').on('select2:select', function (e) {
        var data = e.params.data;

        $('#saleinfo').find('#samas_salecode').val(data.code).trigger('change');
        $('#saleinfo').find('#samas_salename').val(data.lname).trigger('change');
        $('#saleinfo').find('#samas_salegroup').val(data.salegroup).trigger('change');
        $('#saleinfo').find('#samas_salechannel').val(data.salechannel).trigger('change');
        $('#saleinfo').find('#salegrch_branch').val(data.branch).trigger('change');
        $('#saleinfo').find('#salegrch_area').val(data.area).trigger('change');
        $('#saleinfo').find('#salegrch_area_sale').val(data.sales_area).trigger('change');
        $('#saleinfo').find('#samas_salemanager').val(data.store_manager).trigger('change');
        $('#saleinfo').find('#samas_salesupervisorsupport').val(data.support_supervisor).trigger('change');
        $('#saleinfo').find('#samas_salesupport1').val(data.sale_support1).trigger('change');
        $('#saleinfo').find('#samas_salesupport2').val(data.sale_support2).trigger('change');
    ...
    $('#emmas_eline').on('select2:select, change', function (e) {
    ...
    fetch(banklist_get).then(function (response) {
        ...
        $.each(result.data, function (key, val) {
            ...
            if (val['record_status'] == '1' && (val['bank_id'] != "B0006" && val['bank_id'] != "B0008")) { //hide TMB กับ ธนชาต 
                bank_list.push({ id: val['bank_id'], text: val['bank_alias'] });
                bank_multilist.push({ value: val['bank_id'], label: val['bank_alias'] });   //kung20/10/2022
            }
        ...
        bank_multiselect = new Choices('#salegrch_banklist', {
            removeItemButton: true,
            choices: bank_multilist
        });

        /* end kung edit 20/10/2022  */
    ...
    $('#modal-frm_data').on('shown.bs.modal', function (e) {
        $(".main-nav-line li").find("a").eq(0).trigger('click')
    })
    ...
    $(".province").off().on('change', function (e) {
        ...
        if ($(this).attr('id') == 'emmas_eprovinc') {
            ...
            $('#salegrch_ezip2').val('');
            salegrch_eprovinc2_val = $(this).val();
            ...
            $('#emmasaddr_ezip').val('');
            emmasaddr_eprovinc_val = $(this).val();
    ...
    $("input.numbers,input.number").keypress(function (event) {
    ...
    $('#modal-addcontact').on('hide.bs.modal', function () {
        $('#frm_contact').find('input, select').val('').trigger('change')
        $('#frm_contact').parsley().reset();
        $('#btn-contactinfo-create').removeClass('d-none');
        $('#btn-contactinfo-update').addClass('d-none');
    ...
    $('#btn_add_master_route').on('click', function () {
        window.open('/stl/master/delivery_zone?window=new', 'delivery_zone', "status=1,toolbar=1");
        return false;
    })

    $('#modal-owner').on('hide.bs.modal', function () {
        $('#frm_owner').parsley().reset();
    });

    $('#modal-bankaccount').on('hide.bs.modal', function () {
        $('#frm_bankaccount').parsley().reset();
        $('#frm_bankaccount input , #frm_bankaccount select').val('').trigger('change')
    });
...
$.List_cus_addr_187 = async function (cusitem) {
    ...
    $('#modal-addr').on('hide.bs.modal', function () {
        $('#frm_customer').find('input, select').val('').trigger('change').prop("disabled", false).prop("readonly", false)
        $('#frm_customer').find('.default-switches').removeClass('on');
        $('#frm_customer').parsley().reset();
                ...
                $.contextMenu({
                    ...
                    callback: function (key, options) {
                        ...
                        if (key === 'view') {
                            ...
                            $('#saleinfo').find('#search_samas_sale').prop("disabled", false);
                            $('#saleinfo').find('#samas_target, #samas_newcust').prop("disabled", false);
...
$.Details = async function (citem) {
    ...
    $('#customerinfo').find('#btn_addowner').on('click', function () {
        ...
        $('#btn-owner-update').addClass('d-none')
    ...
    $('#accountinfo').find('#btn_addbankaccount').on('click', function () {
        ...
        $('#btn-bankaccount-update').addClass('d-none')
    ...
    fetch(customer_bankaccount_get + '?item_code=' + citem['code'] +'&record_status=1').then(function (response) {
        return response.json();
    }).then(function (result) {
        if (result.data.length == 0) {
    ...
    $('#btn_addbankaccount').prop('disabled', true);
    $('#btn_addowner').prop('disabled', true);
    $('.bankacc-action').prop('disabled', true);
    $('.owner-action').prop('disabled', true);
    $('#btn_addaddress').prop('disabled', true);
    $('#btn_addcontact').prop('disabled', true);
...
$.Update = async function (citem) {
    ...
    $('#btn_addbankaccount').prop('disabled', false);
    $('#btn_addowner').prop('disabled', false);
    $('.bankacc-action').prop('disabled', false);
    $('.owner-action').prop('disabled', false);
    $('#btn_addaddress').prop('disabled', false);
    $('#btn_addcontact').prop('disabled', false);
...
$.Customer_Update = async function (emmas_code) {
    ...
    let update_data = {
        ...
        emmas_esale: $('#saleinfo').find('#samas_salecode').val(),
        emmas_esalegroup: $('#saleinfo').find('#samas_salegroup').val(),

        salegrch_salecode: $('#saleinfo').find('#samas_salecode').val(),
        salegrch_salename: $('#saleinfo').find('#samas_salename').val(),
        salegrch_salegroup: $('#saleinfo').find('#samas_salegroup').val(),
        salegrch_salechannel: $('#saleinfo').find('#samas_salechannel').val(),
        salegrch_branch: $('#saleinfo').find('#salegrch_branch').val(),
        salegrch_area: $('#saleinfo').find('#salegrch_area').val(),
        salegrch_salearea: $('#saleinfo').find('#salegrch_area_sale').val(),
        salegrch_storemanager: $('#saleinfo').find('#samas_salemanager').val(),
        salegrch_salesupervisorsupport: $('#saleinfo').find('#samas_salesupervisorsupport').val(),
        salegrch_salesupport1: $('#saleinfo').find('#samas_salesupport1').val(),
        salegrch_salesupport2: $('#saleinfo').find('#samas_salesupport2').val(),

        salegrch_target: $('#saleinfo').find('#samas_target').val(),
        salegrch_newcust: $('#saleinfo').find('#samas_newcust').val(),
...
$.SaleTarget_Update = async function (emmas_code) {
    ...
    let update_data = {
        ...
        emmas_code: emmas_code,
        saletargetcsum1_salestarget_year: $('#saleinfo').find('#saletargetcsum1_salestarget_year').val(),
...
$.CreateContact = async function (citem) {
    ...
    $('#btn-contactinfo-create').on('click', function () {
        $('#frm_contact').parsley().on('form:submit', function () {
...
$.CreateAddr_187 = async function (cusitem) {
    ...
    $('#btn-item-create').off().on('click', function (e) {
        ...
        if ($('#frm_customer').parsley().isValid()) {
            ...
            let no = $('#emmasaddr_houseno').val() != '' ? 'เลขที่ ' + $('#emmasaddr_houseno').val() : '';
            let alley_no = $('#emmasaddr_villageno').val() != '' ? ' หมู่ ' + $('#emmasaddr_villageno').val() : '';
            let alley = $('#emmasaddr_village').val() != '' ? ' หมู่บ้าน ' + $('#emmasaddr_village').val() : '';
            let soi = $('#emmasaddr_lane').val() != '' ? ' ซอย ' + $('#emmasaddr_lane').val() : '';
            let road = $('#emmasaddr_streetname').val() != '' ? ' ถนน ' + $('#emmasaddr_streetname').val() : '';
            let emmasaddr_eaddress = no + alley_no + alley + soi + road

            // Model & Repo ไปเปลี่ยนเอาเอง
            let add_data = {
                emmasaddr_emmas_code: cusitem['code'],
                emmasaddr_location_name: $('#emmasaddr_location_name').val(),
                emmasaddr_eaddress: emmasaddr_eaddress,

                emmasaddr_houseno: $('#emmasaddr_houseno').val(),
                emmasaddr_villageno: $('#emmasaddr_villageno').val(),
                emmasaddr_lane: $('#emmasaddr_lane').val(),
                emmasaddr_village: $('#emmasaddr_village').val(),
                emmasaddr_streetname: $('#emmasaddr_streetname').val(),
...
$.DetailsAddr_187 = async function (citem, cusitem) {
    ...
    $('#emmasaddr_houseno').val(citem['emmasaddr_houseno']);
    $('#emmasaddr_villageno').val(citem['emmasaddr_villageno']);
    $('#emmasaddr_lane').val(citem['emmasaddr_lane']);
    $('#emmasaddr_village').val(citem['emmasaddr_village']);
    $('#emmasaddr_streetname').val(citem['emmasaddr_streetname']);
...
$.List_Delivery = async function (addrcitem, cusitem) {
    ...
    $('#frm_trans input,#frm_trans select').val('').trigger('change')

    $('.btn-create-trp').removeClass('d-none');
    $('.btn-edit-trp').addClass('d-none');
    ...
    fetch(url).then(function (response) {
        ...
        if (result.status === 'Error') {
            ...
            flex_vendor = result.data.length;
    ...
    $('#btn-item-trp-cancle').on('click', function () {
        ...
        $('#emmas_eline').val('').trigger('change')
...
$.Edit_Trp = async function (citem, addrcitem, cusitem) {
    ...
    $('.btn-create-trp').addClass('d-none');
    $('.btn-edit-trp').removeClass('d-none');
...
$.Details_Trp = async function (citem) {
    ...
    $('#emmas_eline').val(citem['lov_zone_code']).trigger('change.select2');
    $('#tran_name').val(citem['vendor_id']).trigger('change.select2');
...
$.Contact_Detail_Get = async function (ContactData) {
    ...
    $.each(ContactData, function (key, val) {
        ...
        let purpose_code
        let contact_data = JSON.stringify(val);
        ...
        if (val['record_status'] == '1') {
            $('#frm-contact-info').append('<div class="row row-sm ' + margin + '"><div class="col-lg">'
                ...
                + '<div class="col-lg mg-t-10 mg-lg-t-0"><label class="tx-13 tx-medium tx-gray-700 ' + classShow + '" style="padding-left: 5px;">Extension</label>'
                ...
                + '<div class="col-lg mg-t-10 mg-lg-t-0"><label class="tx-13 tx-medium tx-gray-700 ' + classShow + '" style="padding-left: 5px;">Contact name</label>'
                ...
                + '<div class="col-lg mg-t-10 mg-lg-t-0"><label class="tx-13 tx-medium tx-gray-700 ' + classShow + '" style="padding-left: 5px;">Purpose</label>'
                ...
                + '<div class="col-lg col-lg-4 mg-t-10 mg-lg-t-0"><label class="tx-13 tx-medium tx-gray-700 ' + classShow + '" style="padding-left: 5px;">Description</label>'
                + '<input class="form-control form-control-sm" placeholder="Description" type="text" id="" style="margin-top: -3px;" value="' + description + '" readonly></div>'

                // start button action
                + '<div class="col-lg d-flex justify-content-center" style="max-width: 5% !important;"><div class="dropdown dropup">'
                + '<button class="btn p-0 bankacc-action" disabled id="dropdownMenuButton-' + key + '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-edit text-danger" style="position: absolute;bottom: 10px;"></i></button>'
                + '<div class="dropdown-menu" aria-labelledby="dropdownMenuButton-' + key + '">'
                + `<a class="dropdown-item" onclick='$.UpdateContact(${contact_data}); return false; '>Edit</a>`
                + `<a class="dropdown-item" onclick='$.DeleteContact(${contact_data}); return false;'>Delete</a>`
                + '</div></div></div>'
                // end button action
...
$.UpdateContact = async function (citem) {
    $('#modal-addcontact').modal('show');

    $('#btn-contactinfo-create').addClass('d-none');
    $('#btn-contactinfo-update').removeClass('d-none');

    $('#contact_number').val(citem['contactnumber']);
    $('#contact_extension').val(citem['extension']);
    $('#contact_name').val(citem['contactname']);
    $('#contact_purpose_code').val(citem['purpose_code']).trigger('change');
    $('#contact_purpose_desc').val(citem['purpose_desc']);
    $('#contact_description').val(citem['description']);

    $('#btn-contactinfo-update').on('click', function () {
        $('#frm_contact').parsley().on('form:submit', function () {

            $('.btn-save_form').prop('disabled', true);

            let add_data = {
                customer_contact_id: citem['customer_contact_id'],
                code: citem['code'],
                contactnumber: $('#contact_number').val(),
                contactname: $('#contact_name').val(),
                extension: $('#contact_extension').val(),
...
$.DeleteContact = async function (citem) {

    swal({
        title: "Are you sure?",
        text: "This information will be deleted immediately.",
        type: "warning",
        showCancelButton: true,
        canceButtonClass: "btn-danger",
    ...
    fetch(customer_profile_detail187_get + '?code=' + emmas_code).then(function (response) {
        ...
        if (result.length > 0) {
            ...
            $.each(result.data, function (key, val) {
                ...
                $('#customerinfo').find('#emmas_ebgservice').val(moment(val['emmas_startdate']).format('DD/MM/YYYY')).prop('disabled', true);
                ...
                $('#customerinfo').find('#emmas_eregdate').val(val['emmas_eregdate'] === null ? '' : moment(val['emmas_eregdate'], 'YYYY-MM-DD').format('DD/MM/YYYY'));
                ...
                $('#customerinfo').find('#salegrch_customer_houseno').val(val['salegrch_customer_houseno']);
                $('#customerinfo').find('#salegrch_customer_villageno').val(val['salegrch_customer_villageno']);
                $('#customerinfo').find('#salegrch_customer_village').val(val['salegrch_customer_village']);
                $('#customerinfo').find('#salegrch_customer_lane').val(val['salegrch_customer_lane']);
                $('#customerinfo').find('#salegrch_customer_streetname').val(val['salegrch_customer_streetname']);

                $('#customerinfo').find('#salegrch_customer_houseno2').val(val['salegrch_customer_houseno2']);
                $('#customerinfo').find('#salegrch_customer_villageno2').val(val['salegrch_customer_villageno2']);
                $('#customerinfo').find('#salegrch_customer_village2').val(val['salegrch_customer_village2']);
                $('#customerinfo').find('#salegrch_customer_lane2').val(val['salegrch_customer_lane2']);
                $('#customerinfo').find('#salegrch_customer_streetname2').val(val['salegrch_customer_streetname2']);
                ...
                var banklist_val = val['salegrch_banklist'];
                var arr_banklist = (banklist_val != null ? banklist_val.split(',') : []);

                bank_multiselect.setChoiceByValue(arr_banklist); //kung edit รายการธนาคารของลูกค้า 20/10/2022
                ...
                $('#saleinfo').find('#samas_salecode').val(val['salegrch_salecode']);
                $('#saleinfo').find('#samas_salename').val(val['salegrch_salename']);
                $('#saleinfo').find('#emmas_egdis').val(val['emmas_egdis']);
                $('#saleinfo').find('#emmas_netprice').val(val['emmas_netprice']);
                $('#saleinfo').find('#samas_storemanager').val(val['salegrch_storemanager']);
                $('#saleinfo').find('#samas_salechannel').val(val['salegrch_salechannel']);
                $('#saleinfo').find('#salegrch_branch').val(val['salegrch_branch']);
                $('#saleinfo').find('#samas_salegroup').val(val['salegrch_salegroup']);
                $('#saleinfo').find('#salegrch_area').val(val['salegrch_area']);
                $('#saleinfo').find('#salegrch_area_sale').val(val['salegrch_salearea']);
                $('#saleinfo').find('#salegrch_salearea').val(val['salegrch_salearea']);
                $('#saleinfo').find('#samas_salemanager').val(val['salegrch_storemanager']);
                ...
                $('#saleinfo').find('#samas_salesupervisorsupport').val(val['salegrch_salesupervisorsupport']);
                $('#saleinfo').find('#samas_salesupport1').val(val['salegrch_salesupport1']);
                $('#saleinfo').find('#samas_salesupport2').val(val['salegrch_salesupport2']);
                $('#saleinfo').find('#samas_cod').val(val['emmas_ekcust']);
                $('#saleinfo').find('#samas_target').val(val['salegrch_target']);
                $('#saleinfo').find('#samas_newcust').val(val['salegrch_newcust']);
                var $newOption = $("<option selected='selected'></option>").val(val['salegrch_salecode']).text(val['saletext'])
                $("#search_samas_sale").append($newOption).trigger('change');
            ...
            setTimeout(function () {
                toastr.error('ไม่พบข้อมูลลูกค้านี้ในตารางข้อมูลลูกค้า');
            }, 100);
    ...
    fetch(customer_profile_saleTarget187_get + '?code=' + emmas_code + '&target_year=' + year).then(function (response) {
        ...
        if (result.length > 0) {
            ...
            $.each(result.data, function (key, val) {
                ...
                //$('#saleinfo').find('#saletargetcsum1_salestarget_year').val(val['saletargetcsum1_salestarget_year']).trigger('change');
                ...
                let saletargetcsum1_salesactual_m01 = (Math.round(val['saletargetcsum1_salesactual_m01'] * 100) / 100).toFixed(2)
                ...
                //$('#saleinfo').find('#saletargetcsum1_salestarget_year').text(numberWithCommas(saletargetcsum1_salestarget_year));
                ...
                $('#saleinfo').find('#saletargetcsum2_rebatestarget_q1_per').text('(' + numberWithCommas(saletargetcsum2_rebatestarget_q1_per) + '%)');
                ...
                $('#saleinfo').find('#saletargetcsum1_salesactual_m01').text(numberWithCommas(saletargetcsum1_salesactual_m01));
                $('#saleinfo').find('#saletargetc_salestarget_m06').text(numberWithCommas(saletargetc_salestarget_m06));
                $('#saleinfo').find('#saletargetc_salestarget_m07').text(numberWithCommas(saletargetc_salestarget_m07));
                $('#saleinfo').find('#saletargetcsum1_salesactual_m02').text(numberWithCommas(saletargetcsum1_salesactual_m02));
                $('#saleinfo').find('#saletargetc_salestarget_m08').text(numberWithCommas(saletargetc_salestarget_m08));
                ...
                $('#saleinfo').find('#saletargetc_salestarget_m11').text(numberWithCommas(saletargetc_salestarget_m11));
                ...
                //let saletargetcsum1_salesactual_m01 = (Math.round(val['saletargetcsum1_salesactual_m02'] * 100) / 100).toFixed(2)
                //let saletargetcsum2_salestarget_m01_per = (Math.round(val['saletargetcsum2_salestarget_m01_per'] * 100) / 100).toFixed(2)
                //let saletargetcsum1_salesactual_m02 = (Math.round(val['saletargetcsum1_salesactual_m02'] * 100) / 100).toFixed(2)
                //let saletargetcsum2_salestarget_m02_per = (Math.round(val['saletargetcsum2_salestarget_m02_per'] * 100) / 100).toFixed(2)
                //let saletargetcsum1_salesactual_m03 = (Math.round(val['saletargetcsum1_salesactual_m03'] * 100) / 100).toFixed(2)
            ...
            setTimeout(function () {
                toastr.error('ไม่พบข้อมูล Sale Target นี้ในตารางข้อมูลลูกค้า');
...
$.CreateOwner = async function (citem) {
    ...
    $('#btn-owner-create').on('click', function () {
        $('#frm_owner').parsley().on('form:submit', function () {

            //alert("CreateOwner - Start");
            //return false;

            $('.btn-save-owner').prop('disabled', true);
            ...
            fetch(customer_owner_create, {
                ...
                if (data.status === 'Error') {
                    ...
                    if (data.data[0]['pMessage'] == 'Insert Data Successfully') {
                        toastr.success('Save Successfully!');
                    } else {
                        swal({
                            title: "หมายเลขบัตรประชาชนผู้มีอำนาจหรือเจ้าของกิจการซ้ำ!",
                            text: "หมายเลขบัตรประชาชนนี้มีอยู่ในระบบเรียบร้อยแล้ว!",
                            type: 'error',
                            //timer: 1000,
                    ...
                    $('.btn-save-owner').prop('disabled', false);
...
$.DeleteOwner = async function (citem) {
...
$.UpdateOwner = async function (citem) {
    $('#modal-owner').modal('show')

    $('#btn-owner-create').addClass('d-none')
    $('#btn-owner-update').removeClass('d-none')

    $('#customer_owner_name').val(citem['owner_name']);
    $('#customer_owner_idno').val(citem['owner_idno']);

    $('#btn-owner-update').on('click', function () {
        $('#frm_owner').parsley().on('form:submit', function () {
    ...
    $.each(OwnerData, function (key, val) {
        ...
        if (val['record_status'] == '1') {
            $('#frm-owner-info').append('<div class="row row-sm ' + margin + '"><div class="col-lg col-lg-6">'
                ...
                + '<input class="form-control form-control-sm" placeholder="ชื่อผู้มีอำนาจ" type="text" id="name-owm-' + key + '" style="margin-top: -3px;" value="' + owner_name + '" readonly></div>'
                ...
                + '<input class="form-control form-control-sm" placeholder="หมายเลขบัตรประชาชนผู้มีอำนาจ" type="text" id="idcard-owm-' + key + '" style="margin-top: -3px;" value="' + owner_idno + '" readonly></div>'
                //        + `<div class='col-lg col-lg-1'><button class='btn p-0 del-own' disabled onclick='$.DeleteOwner(${result}); return false;'><i class='fas fa-trash text-danger' style='position: absolute;bottom: 10px;'></i></button></div>`

                //start button action
                + '<div class="col-1 d-flex justify-content-center" style="max-width: 5% !important;"><div class="dropdown dropup">'
                + '<button class="btn p-0 owner-action" disabled id="dropdownMenuOwner-' + key + '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-edit text-danger" style="position: absolute;bottom: 10px;"></i></button>'
                + '<div class="dropdown-menu" aria-labelledby="dropdownMenuOwner-' + key + '">'
                + `<a class="dropdown-item owner-edit" id="edit-cus-owner-${key}" onclick='$.UpdateOwner(${result}); return false; '>Edit</a>`
                + `<a class="dropdown-item owner-delete" id="delete-cus-owner-${key}" onclick='$.DeleteOwner(${result}); return false;'>Delete</a>`
                + '</div></div></div>'
                // end button action
...
$.CreateBankAccount = async function (citem) {
    ...
    $('#btn-bankaccount-create').on('click', function () {
        $('#frm_bankaccount').parsley().on('form:submit', function () {

            $('.btn-save_bankaccount').prop('disabled', true);
            ...
            fetch(customer_bankaccount_create, {
                ...
                if (data.status === 'Error') {
                    ...
                    if (data.data[0]['pMessage'] == 'Insert Data Successfully') {
                        ...
                        swal({
                            title:"เลขที่บัญชีซ้ำ!",
                            text: "บัญชีนี้มีอยู่ในระบบเรียบร้อยแล้ว!",
                            type: 'error',
                            //timer: 1000,
                            showConfirmButton: true
                        });
                    ...
                    $('.btn-save_bankaccount').prop('disabled', false);
...
$.BankAccount_Detail_Get = async function (BankAccountData) {
    ...
    $.each(BankAccountData, function (key, val) {
        ...
        let cusbankacc_data = JSON.stringify(val);
        ...
        if (val['record_status'] == '1') {
            ...
            $('#frm-bankaccount-info').append('<div class="row row-sm ' + margin + '"><div class="col-lg col-lg-3">'
                ...
                + '<input class="form-control form-control-sm" id="customer-bank-' + key + '" placeholder="ธนาคาร" type="text" id="" style="margin-top: -3px;" value="' + customer_bac_bank + '" readonly></div>'

                + '<div class="col-lg mg-t-10 mg-lg-t-0"><label class="tx-13 tx-medium tx-gray-700 ' + classShow + '" style="padding-left: 5px;">ชื่อบัญชีธนาคาร</label>'
                + '<input class="form-control form-control-sm" id="customer-bankname-' + key + '" placeholder="ชื่อบัญชีธนาคาร" type="text" id="" style="margin-top: -3px;" value="' + customer_bac_bankname + '" readonly></div>'

                + '<div class="col-lg mg-t-10 mg-lg-t-0"><label class="tx-13 tx-medium tx-gray-700 ' + classShow + '" style="padding-left: 5px;">เลขที่บัญชีธนาคาร</label>'
                + '<input class="form-control form-control-sm" id="customer-bankno-' + key + '" placeholder="เลขที่บัญชีธนาคาร" type="text" id="" style="margin-top: -3px;" value="' + customer_bac_bankno + '" readonly></div>'

                + '<div class="col-lg mg-t-10 mg-lg-t-0"><label class="tx-13 tx-medium tx-gray-700 ' + classShow + '" style="padding-left: 5px;">ประเภทบัญชีธนาคาร</label>'
                + '<input class="form-control form-control-sm" id="customer-banktype-' + key + '" placeholder="ประเภทบัญชีธนาคาร" type="text" id="" style="margin-top: -3px;" value="' + customer_bac_type + '" readonly></div>'

                + '<div class="col-lg mg-t-10 mg-lg-t-0"><label class="tx-13 tx-medium tx-gray-700 ' + classShow + '" style="padding-left: 5px;">หมายเหตุ</label>'
                + '<input class="form-control form-control-sm" id="customer-bankcomment-' + key + '" placeholder="หมายเหตุ" type="text" id="" style="margin-top: -3px;" value="' + customer_bac_comment + '" readonly></div>'

                //start button action
                + '<div class="col-1 d-flex justify-content-center" style="max-width: 5% !important;"><div class="dropdown dropup">'
                + '<button class="btn p-0 bankacc-action" disabled id="dropdownMenuButton-' + key + '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-edit text-danger" style="position: absolute;bottom: 10px;"></i></button>'
                + '<div class="dropdown-menu" aria-labelledby="dropdownMenuButton-' + key + '">'
                + `<a class="dropdown-item cusbankacc-edit" id="edit-cus-bankacc-${key}" onclick='$.UpdateCusBankAcc(${cusbankacc_data}); return false; '>Edit</a>`
                + `<a class="dropdown-item cusbankacc-delete" id="delete-cus-bankacc-${key}" onclick='$.DeleteCusBankAcc(${cusbankacc_data}); return false;'>Delete</a>`
                + '</div></div></div>'
                // end button action
...
$.UpdateCusBankAcc = async function (acc_data) {
    $('#modal-bankaccount').modal('show');

    $('#btn-bankaccount-create').addClass('d-none')
    $('#btn-bankaccount-update').removeClass('d-none')

    $('#customer_bac_bank').val(acc_data['customer_bac_bank_id']).trigger('change');
    $('#customer_bac_bankname').val(acc_data['customer_bac_bankname']);
    $('#customer_bac_bankno').val(acc_data['customer_bac_bankno']);
    $('#customer_bac_type').val(acc_data['customer_bankacc_type']).trigger('change');
    $('#customer_bac_comment').val(acc_data['customer_bac_comment']);

    $('#btn-bankaccount-update').on('click', function () {
        $('#frm_bankaccount').parsley().on('form:submit', function () {

            $('.btn-save_form').prop('disabled', true);

            let add_data = {
                customer_bankaccount_id: acc_data['customer_bankaccount_id'],
                code: acc_data['code'],
                customer_bac_bank: $('#customer_bac_bank option:selected').val(),
                customer_bac_bankname: $('#customer_bac_bankname').val(),
                customer_bac_bankno: $('#customer_bac_bankno').val(),
            ...
            fetch(customer_bankaccount_update, {
                method: 'PUT', // *GET, POST, PUT, DELETE, etc.
                // mode: 'no-cors', // no-cors, *cors, same-origin
                cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
                credentials: 'same-origin', // include, *same-origin, omit
...
$.DeleteCusBankAcc = async function (acc_data) {
    swal({
        title: "Are you sure?",
        text: "This information will be deleted immediately.",
        type: "warning",
        ...
        if (isConfirm) {
            ...
            fetch(customer_bankaccount_delete, {
                method: 'PUT', // *GET, POST, PUT, DELETE, etc.
                // mode: 'no-cors', // no-cors, *cors, same-origin
                cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
                credentials: 'same-origin', // include, *same-origin, omit
                ...
                if (data.status === 'Error') {
                    ...
                    toastr.success('Save Successfully!', async function () {
                        swal({ type: "success", title: 'Your work has been saved', showConfirmButton: false, timer: 1500 })
            ...
            swal("Cancelled", "Your imaginary information is safe :)", "error");
        }
...
$.Route_Get = async function () {
    ...
    let url_lov_route_get_api = new URL(url_lov_route_get);

    fetch(url_lov_route_get_api + '?lov_type=Route_No').then(function (response) {
        ...
        $.each(result.data, function (key, val) {
            //lov_zone.push({ id: val['lov_code'], text: val['lov1'] });
            $('#lov_route').append('<option value="' + val['lov_code'] + '">' + val['lov1'] + '</option>');
        ...
        //    width: '100%',
        //    height: '100%',
        //    dropdownParent: $("#frm_route #lov_route"),
        //    placeholder: 'Select Route No.',
        //    allowClear: false,
        //    data: lov_zone,
        //    templateResult: function (data) {
