...
$.carmodel_sub_list_new = function format(callback,
    ...
    url.search = new URLSearchParams({
        ...
        hign_stant: hign_stant == null ? '' : hign_stant,
        street_name: street_name == null ? '' : street_name,
        wheel_drive: wheel_drive == null ? '' : wheel_drive,
        wheel: wheel == null ? '' : wheel,
        chassis_model: chassis_model == null ? '' : chassis_model,
    });
    ...
    fetch(url).then(function (response) {
        ...
        if (result.length > 0) {
            ...
            $.each(data, function (key, val) {
                ...
                let fuel_type = val.fuel_type == null ? '' : val.fuel_type
                let engine_displacement = val.engine_displacement == null ? '' : val.engine_displacement
                let engine_code = val.engine_code == null ? '' : val.engine_code
                let transmission_type = val.transmission_type == null ? '' : val.transmission_type
                let wheel_drive = val.wheel_drive == null ? '' : val.wheel_drive
                let model_code = val.model_code == null ? '' : val.model_code
