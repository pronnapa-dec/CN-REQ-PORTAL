...
firebase.auth().onAuthStateChanged(function (user) {
    ...
    if (user) {
        ...
        $.Report = async function (citem) {
            ...
            $('#frm_assess').find('#btn-report-assess').off('click').on('click', async function (e) {

                e.preventDefault();

                console.log('Report', citem['data_id'])

                //toastr.success('การประเมินสำเร็จ!');

                setTimeout(async function () {

                    let url_report = "http://192.168.1.187/ReportServer/Pages/ReportViewer.aspx?%2fReport+Project1%2fRPT_HR_ASSESS&rs:Command=Render&data_id=" + citem['data_id'] + "";
