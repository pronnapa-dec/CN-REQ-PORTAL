...
$.List = async function (employee_id) {
    ...
    fetch(url_hr_employee_detail + '?employee_id=' + employee_id).then(function (response) {
        ...
        if (result.status === 'Error') {
            ...
            let emp_assessor_photo = result.data[0]['employee_photo']

            leader_assess_id = result.data[0]['employee_id']

            $('#frm_detail').find('#emp_assessor').html(emp_assessor_code + ' คุณ' + emp_assessor_name);
            $('#frm_detail').find('#emp_assessor_photo').attr('src', emp_assessor_photo == null ? '../../assets/img/ecommerce/01.jpg' : url_image_employee + emp_assessor_photo)
    ...
    fetch(Employee_Get).then(function (response) {
        ...
        if (result.status === 'Error') {
            ...
            $.each(result.data, function (key, val) {
                ...
                let status = val['status_assess'];

                let tx_status = ''
                let color_status = ''

                if (status == 'FALSE') {
                    tx_status = 'ยังไม่ได้รับการประเมิน'
                    color_status = 'danger'
                } else if (status == 'WAIT') {
                    tx_status = 'รอการอนุมัติ'
                    color_status = 'orange'
                } else if (status == 'SUCCEED') {
                    tx_status = 'อนุมัติแล้ว'
                    color_status = 'success'
                } else {
                    tx_status = '-'
                }

                employee_dataSet.push({ id: val['employee_id'], text: val['employee_code'] + ' ' + val['employee_prefix'] + ' ' + val['employee_name'] + ';&nbsp;( ' + '<span class="tx-bold tx-' + color_status + '">' + tx_status + '</span>' + ' ) ' , data: val });
    ...
    fetch(url_hr_employee_detail + '?employee_id=' + employee_id).then(function (response) {
        ...
        if (result.status === 'Error') {
            ...
            $.each(result.data, function (key, val) {
                ...
                $('#frm_detail').find('#h_employee_photo').attr('src', citem['employee_photo'] == null ? '../../assets/img/ecommerce/01.jpg' : url_image_employee + citem['employee_photo'])
        ...
        if (result.status === 'Error') {
            ...
            $.each(result.data, function (key, val) {
                ...
                if (status == 'codedup') {
                    ...
                    $('.chkdup').removeClass('alert-warning');
                    $('.chkdup').addClass('alert-warning');
                    $('.alert-text').html('ท่านได้ทำแบบประเมินแล้ว ');
                    ...
                    $('.chkdup').removeClass('alert-warning');
                    $('.chkdup').addClass('alert-primary');
                    $('.alert-text').html('เริ่มทำการประเมิน ');
                    $('#alert').html(moment().format('DD/MM/YYYY HH:mm:ss') + ' น.');
                    $('.chkdup').removeClass('d-none');
