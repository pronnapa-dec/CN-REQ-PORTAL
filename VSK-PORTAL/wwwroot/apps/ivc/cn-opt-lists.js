                ...
                "initComplete": function (settings, json) {
                    ...
                    $.contextMenu({
                        ...
                        items: {
                            ...
                            "edit1": { name: "Update - เปลี่ยนแปลง 'รับคืน' เป็น 'หน้างาน'", icon: "edit change-status1", disabled: true },
                            "edit2": { name: "Update - เปลี่ยนแปลง 'หน้างาน' เป็น 'รับคืน'", icon: "edit change-status2", disabled: true },
                            "sep1": "---------",
                    ...
                    $("tbody").contextmenu(function (key) {
                        setTimeout(function () {
                            ...
                            if (objProfile[0]['role'] == "เจ้าหน้าที่ (IVC)" && assige == "IVC") {
                                ...
                                $('.edit-detail-trp').removeClass('d-none')
                                $('.edit-detail').addClass('d-none')
                                if (Status_text == "Open" && assige == "TRP") {
                            ...
                            if (user_id === 'ongkarn.s' || user_id === 'potjana.s') {
...
$.Details = async function (citem) {
    ...
    fetch(Get_Detail).then(function (response) {
        ...
        if (result.status === 'Error') {
            ...
            $('#frm_data').find('#driver_id').val(deatailDriver);
            ...
            let rq = objProfile[0]['role'] == "เจ้าหน้าที่ (IVC)" ? 'required' : '';
...
$.Edit = async function (citem) {
    ...
    if (objProfile[0]['role'] == "เจ้าหน้าที่ (IVC)") {
        ...
        $('input[name=item_condition]').attr('required');
    ...
    } else if (status == "open" && (objProfile[0]['role'] == "ผู้ดูแลระบบ (TRP)" || objProfile[0]['role'] == "พนักงานทั่วไป (TRP)")) {
    ...
    if (user_id === 'ongkarn.s' || user_id === 'pronnapa.d' || user_id === 'potjana.s') {
    ...
    $('#btn-save_exit').on('click', function (e) {
        ...
        $('#frm_data').parsley().on('form:submit', function () {
            ...
            //if (status == "open" && (objProfile[0]['role'] == "ผู้ดูแลระบบ (TRP)" || objProfile[0]['role'] == "พนักงานทั่วไป (TRP)") && $('#driver_id').val() != '' ) {
            ...
            let edit_data = {
                ...
                cn_pre_job_detail_driver: $('#frm_data').find('#driver_id').val(),
