...
const lists_get = url_api + '/v1/Cn_On_Job_Claim_Get';
                ...
                "initComplete": function (settings, json) {
                    ...
                    $("tbody").contextmenu(function (key) {
                        setTimeout(function () {
                            ...
                            if (role_code == assige || role_code == "CN-DEV") { 
...
$.Details = async function (citem) {
    ...
    $('#frm_data').find('#cn_pre_job_jobno').val(citem['cn_pre_job_jobno']).prop("disabled", true);

    if (citem['cn_on_job_status'] == "pending") {
        $('#frm_data').find('#cn_on_job_status').val('PT พิจรณาสินค้า').prop("disabled", true); 
    } else if (citem['cn_on_job_status'] == "claimable") {
        $('#frm_data').find('#cn_on_job_status').val('เคลมได้').prop("disabled", true);
    } else if (citem['cn_on_job_status'] == "return") {
        $('#frm_data').find('#cn_on_job_status').val('ไม่รับเคลม').prop("disabled", true);
    } else if (citem['cn_on_job_status'] == "supplier") {
        $('#frm_data').find('#cn_on_job_status').val('ส่งสินค้าให้ Supplier').prop("disabled", true);
    }
...
$.Edit = async function (citem) {
    ...
    $('.btn-save_form').show();
    $('#status_claim').removeAttr('disabled');
    $('.rm_readonly').removeAttr('readonly');
    $('.status_claim').show();
    let cn_on_job_detail_assige
    let cn_on_job_detail_status

    $('#btn-save_exit').click(function (e) {
        if ($('#status_claim').val() == "") {
            cn_on_job_detail_assige = "PT";
            cn_on_job_detail_status = citem['cn_on_job_status'];
        } else if ($('#status_claim').val() == "claimable" || $('#status_claim').val() == "supplier") {
            cn_on_job_detail_assige = "IVC";
            cn_on_job_detail_status = $('#status_claim').val();
        } else if ($('#status_claim').val() == "return") {
        ...
        $('#frm_data').parsley().on('form:submit', function () {
            ...
            let edit_data = {
                ...
                cn_on_job_detail_damage: $('#cn_on_job_detail_damage').val(),
