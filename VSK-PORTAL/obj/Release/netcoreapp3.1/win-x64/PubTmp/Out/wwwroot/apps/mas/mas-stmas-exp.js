...
firebase.auth().onAuthStateChanged(function (user) {
    ...
    if (user) {
                ...
                ajax: {
                    ...
                    processResults: function (data, search) {
                        ...
                        return {
                            results: $.map(data.data, function (item) {
                                //console.log('item', item);
                                code_e_select = item.code.trim()
                                gcode_e_name = item.gname.trim()
                                return {
                                    text: item.code.trim() + ' - ' + item.gname.trim(),
                                    //text: item.gname.trim(),
                                    id: item.code,
                        ...
                        $.each(result.data, function (key, val) {
                            ...
                            $('#gcode_a')
                                .append($("<option>Please select..</option>")
                                    ...
                                    .attr('data-text', val['code_aexp_name']));
                            ...
                            $('#gcode_b')
                                .append($("<option>Please select..</option>")
                                    ...
                                    .attr('data-text', val['code_b_name']));
                            $('#gcode_c')
                                .append($("<option>Please select..</option>")
                                    .attr("value", val['code_c'])
                                    .text(val['code_c'] + ' - ' + val['code_c_name'])
                                    //.text(val['code_c_name'])
                                    .attr('data-text', val['code_c_name']));
                            $('#gcode_d')
                                .append($("<option>Please select..</option>")
                                    .attr("value", val['code_d'])
                                    .text(val['code_d'] + ' - ' + val['code_d_name'])
                                    //.text(val['code_d_name'])
                                    .attr('data-text', val['code_d_name']));
                            $('#gcode_e')
                                .append($("<option>Please select..</option>")
                                    .attr("value", val['code_e'])
                                    .text(val['code_e'] + ' - ' + val['code_e_name'])
                                    //.text(val['code_e_name'])
                                    .attr('data-text', val['code_e_name']));

                            //$('#frm_data').find('#gcode_a').val(val['code_a']).trigger('change').prop('disabled', false);


                            $('#frm_data').find('#gcode_a').trigger('change').data('text', val['code_aexp_name']).prop('disabled', false);
                            $('#frm_data').find('#gcode_b').val(val['code_b']).data('text', val['code_b_name']).trigger('change').prop('disabled', false);
                            $('#frm_data').find('#gcode_c').val(val['code_c']).data('text', val['code_c_name']).trigger('change').prop('disabled', false);
                            $('#frm_data').find('#gcode_d').val(val['code_d']).data('text', val['code_d_name']).trigger('change').prop('disabled', false);
                            $('#frm_data').find('#gcode_e').val(val['code_e']).data('text', val['code_e_name']).trigger('change').prop('disabled', false);
                            ...
                            gcode_a_name = $('#gcode_a').children("option:selected").data('text');
                            gcode_b_name = $('#gcode_b').children("option:selected").data('text');
                            gcode_c_name = $('#gcode_c').children("option:selected").data('text');
                            gcode_d_name = $('#gcode_d').children("option:selected").data('text');
                            gcode_e_name = $('#gcode_e').children("option:selected").data('text');

                            code_e_select = gcode_e;
        ...
        $.Check = async function () {
            ...
            gcode_a_name = $('#gcode_a').data('text');
            gcode_b_name = $('#gcode_b').data('text');
            gcode_c_name = $('#gcode_c').data('text');
            gcode_d_name = $('#gcode_d').data('text');
            gcode_e_name = $('#gcode_e').data('text');

            code_e_select = $('#gcode_e').val();

            //console.log('a: ', gcode_a_name);
            //console.log('b: ', gcode_b_name);
            //console.log('c: ', gcode_c_name);
            //console.log('d: ', gcode_d_name);
            //console.log('e: ', gcode_e_name);


            $('.btn-create').prop('disabled', true);

            $('#gcode_e').on('change', function (e) {

                e.preventDefault()

                code_e_select = $(this).val();

                fetch(url_stmas_exp_get + '?Mode=' + 'code_5' + '&code=' + code_e_select).then(function (response) {
                    return response.json();
                }).then(function (result) {
                    //alert(code_e)
            ...
            await $('#frm_data').find('.gcode').off('select2:select').on('select2:select', function (evt) {
                ...
                $('#gcode_c').on('select2:select', function (e) {
                    ...
                    gcode_c_name = data.name

                });

                $('#gcode_d').on('select2:change', function (e) {
                    var data = e.params.data;
                    $("#gcode_d option[value=" + data.id + "]").attr('data-text', data.name);
                    $("#gcode_d").trigger('change');
                    gcode_d_name = data.name
                });
                ...
                $('#frm_data').find('#item_code').val(gcode_code).trigger('change').prop('disabled', true);
                $('#frm_data').find('#item_name').val(gcode_name).trigger('change').prop('disabled', true);
        ...
        $.Details = async function (citem) {
            ...
            $('#frm_data').find('#gcode_a').val(citem['code_aexp']).attr('data-text', citem['code_aexp']).prop('disabled', true);
            $('#frm_data').find('#gcode_b').val(citem['code_b']).attr('data-text', citem['code_b']).prop('disabled', true);
            $('#frm_data').find('#gcode_c').val(citem['code_c']).attr('data-text', citem['code_c']).prop('disabled', true);
            $('#frm_data').find('#gcode_d').val(citem['code_d']).attr('data-text', citem['code_d']).prop('disabled', true);
            $('#frm_data').find('#gcode_e').val(citem['code_e']).attr('data-text', citem['code_e']).prop('disabled', true);
