...
$.init = function () {
    ...
    console.log(application_dataSet);

    // Start Menu Parent List
    $.LoadMenuParentList = function (application_id, menu_parent) {

        $('#menu_parent').empty().select2().select2("destroy");

        let menu_parent_query = fs.collection('auth_menu')
            .where("record_status", "in", ["Y", "N"])
            .where("menu_parent", "==", null)
            .where("application_id", "==", application_id);

        menu_parent_query.get().then(function (querySnapshot) {

            let menu_parent_dataSet = [];

            querySnapshot.forEach(function (doc) {

                menu_parent_dataSet.push({ id: doc.data().menu_id, text: doc.data().menu_name });
            ...
            $('#menu_parent').select2({
                ...
                placeholder: {
                    id: '0', // the value of the option
                    text: '--- Select Menu Parent ---'
                },
                allowClear: true,
                data: menu_parent_dataSet,
                templateResult: function (data) {
                    return data.text;
                },
                sorter: function (data) {
            ...
            $('#menu_parent').val(menu_parent).trigger("change");

        });
    };
    // End Menu Parent List
    ...
    query.get().then(function (querySnapshot) {
        ...
        querySnapshot.forEach(function (doc) {
            ...
            dataSet.push([
                ...
                doc.data().menu_parent,         // 1
                doc.data().menu_code,           // 2
                doc.data().menu_name,           // 3 
                doc.data().menu_icon,           // 4
                doc.data().menu_url,            // 5
                doc.data().menu_order,          // 6 
                doc.data().record_status,       // 7
                doc.data().menu_id,             // 8 
                doc.data().created_by,          // 9
                doc.data().created_date,        // 10
                doc.data().updated_by,          // 11
                doc.data().updated_date         // 12
            ]);
        ...
        oTable = $('#tbl-list').DataTable({
            ...
            "rowCallback": function (row, data) {
                ...
                fs.collection('auth_menu').where("menu_id", "==", data[1]).limit(1).get().then(function (querySnapshot) {

                    querySnapshot.forEach(function (doc) {

                        $('td:eq(1)', row).html(doc.data().menu_name);
...
$.Create = async function () {
    ...
    $('#application_id').on('select2:select', function (e) { $.LoadMenuParentList($('#application_id').val(), "0"); });
    ...
    $('.btn-save_form').click(function (e) {
        ...
        $('#frm_data').parsley().on('form:submit', function () {
            ...
            fs.collection(collection).doc(uuid).set(menu_citem).then(function () {
...
$.Details = async function (citem) {
    ...
    $('#menu_code').val(citem['menu_code']).prop('disabled', true);
    $('#menu_name').val(citem['menu_name']).prop('disabled', true);
    $('#menu_icon').val(citem['menu_icon']).trigger("change").prop('disabled', true);
    $('#menu_url').val(citem['menu_url']).prop('disabled', true);
    $('#menu_order').val(citem['menu_order']).prop('disabled', true);

    $.LoadMenuParentList(citem['application_id'], citem['menu_parent']);

    $('#menu_parent').prop('disabled', true);
...
$.Edit = async function (citem) {
    ...
    $('#application_id').on('select2:select', function (e) {
        $.LoadMenuParentList($('#application_id').val(), "0");
    });
    ...
    $('#btn-save_exit').click(function (e) {
        ...
        $('#frm_data').parsley().on('form:submit', function () {
            ...
            fs.collection(collection).doc(citem['menu_id']).update(menu_citem).then(function () {
