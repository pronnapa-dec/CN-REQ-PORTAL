...
let lists_get = 'http://192.168.1.247/intranet/pur-api/v1/Cn_Pre_Job_Get';
let lists_history = 'http://192.168.1.247/intranet/pur-api/v1/Cn_Pre_Job_Detail_Get';
let update_detail = 'http://192.168.1.247/intranet/pur-api/v1/Cn_Pre_Job_Detail_Create';
let export_invoice = 'http://192.168.1.247/intranet/report/cn-rpt/Pages/RPT_CN/RPT_CN_REINVOICE';
...
$.Edit = async function (citem) {
    ...
    if (role_code == "IVC") {
    ...
    } else if (role_code == "TRP" || role_code == "CN-ADMIN") {
        if (role_code == "TRP") {
            $('#frm_data').find('#cn_pre_job_remark_trp').removeAttr('disabled');
        } else {
            $('#frm_data').find('#cn_pre_job_remark_trp').prop('disabled', true);
        }
        if (status == "open" && cn_type == "1") {
            ...
            cn_pre_job_detail_assige = "TRP" // เมื่อ TRP เข้ารับของไม่ได้
            cn_pre_job_detail_status = "on_process"
        } else {
            cn_pre_job_detail_status = $('#job_status_edit option:selected').val();
        }
