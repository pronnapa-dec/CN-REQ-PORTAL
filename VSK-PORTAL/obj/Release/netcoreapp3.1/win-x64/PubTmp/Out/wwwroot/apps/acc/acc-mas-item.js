...
let dimension_get = connect_url + '/v1/itemmaster_dimension_get';
let errorlog_get = connect_url + '/v1/itemmaster_errorlog_get';
let itemmaster_update = connect_url + '/v1/itemmaster_update';
...
$.init = function () {
    ...
    //$('#search_item_code_2').val('102401-H366-1949-000-3844');
    ...
    fetch(lov_get + '?lov_code=' + username + '&lov_group=Auth&lov_type=ItemMaster Information').then(function (response) {
    ...
    $('#modal-frm_data').on('hidden.bs.modal', function () {
        ...
        $('#dimension').find('input').val('');
        $('#dimension').find('.tx-value').text('');
            ...
            $.each(result.data, function (key, val) {
                ...
                fetch(product_purplan_get + '?item_code=' + citem['code']).then(function (response) {
                    ...
                    $.each(result.data, function (key, val) {
                        ...
                        var prodpurplan_stdmargin = parseFloat(val['StdMargin']).toFixed(2);
                        ...
                        $('#stockprice').find('#prodpurplan_stdmargin').val($.addCommas(prodpurplan_stdmargin));
                        ...
                        if (val['GR_VAT'] == '0' || val['GR_VAT'] == 0) {
                            $('#inventory1').find('#prodpurplan_grvat_exvat').prop("checked", true);
                        } else if (val['GR_VAT'] == '1' || val['GR_VAT'] == 1) {
                            $('#inventory1').find('#prodpurplan_grvat_invat').prop("checked", true);
                        } else if (val['GR_VAT'] == '0' || val['GR_VAT'] == 0) {
                            $('#inventory1').find('.prodpurplan_grvat').prop("checked", false);
                        }
                        ...
                        $('#inventory2').find('#prodpurplan_sourcetypereviewdate').text(val['SourceType_ReviewDate']);
                            ...
                            fetch(goodprice_get + '?item_code=' + citem['code']).then(function (response) {
                                ...
                                fetch(role_get + '?auth_username=' + prodpurplan_updatedby2).then(function (response) {
                                    ...
                                    if (result.length > 0) {
                                        ...
                                        console.log("count_auth_update", count_auth_update);

                                        if (count_auth_update == 0) {
                                            $('#btn-save_exit').prop("disabled", true);
                                        } else if (count_auth_update > 0) {
                                            $('#btn-save_exit').prop("disabled", false);
                                        }
                                    ...
                                    fetch(dimension_get + '?item_code=' + citem['code']).then(function (response) {

                                        return response.json();

                                    }).then(function (result) {

                                        //console.log("dimension_get", result.data);

                                        if (result.length > 0) {
                                            $.each(result.data, function (key, val) {

                                                var val_yes = '<a class="badge badge-success" href="#" style="width: 45px;">Yes</a>';
                                                var val_no = '<a class="badge badge-danger" href="#" style="width: 45px;">No</a>';

                                                if (val['bigsize'] == 1 || val['bigsize'] == "1") { var var_bigsize = val_yes; } else if (val['bigsize'] == 0 || val['bigsize'] == "0"){ var var_bigsize = val_no; }
                                                if (val['careful'] == 1 || val['careful'] == "1") { var var_careful = val_yes; } else if (val['careful'] == 0 || val['careful'] == "0") { var var_careful = val_no; }
                                                if (val['chem'] == 1 || val['chem'] == "1") { var var_chem = val_yes; } else if (val['chem'] == 0 || val['chem'] == "0") { var var_chem = val_no; }
                                                if (val['exp'] == 1 || val['exp'] == "1") { var var_exp = val_yes; } else if (val['exp'] == 0 || val['exp'] == "0") { var var_exp = val_no; }
                                                if (val['special'] == 1 || val['special'] == "1") { var var_special = val_yes; } else if (val['special'] == 0 || val['special'] == "0") { var var_special = val_no; }

                                                $('#dimension').find('#dim_width').val(val['width']);
                                                $('#dimension').find('#dim_long').val(val['length']);
                                                $('#dimension').find('#dim_height').val(val['height']);
                                                $('#dimension').find('#dim_uom_qty').val(val['uom_qty']);
                                                $('#dimension').find('#dim_weight').val(val['weight']);

                                                $('#dimension').find('#dim_packing').html(val['packing2']);

                                                $('#dimension').find('#dim_exp').append(var_exp);
                                                $('#dimension').find('#dim_careful').append(var_careful);
                                                $('#dimension').find('#dim_special').append(var_special);
                                                $('#dimension').find('#dim_chem').append(var_chem);
                                                $('#dimension').find('#dim_bigsize').append(var_bigsize);

                                                $('#dimension').find('#dim_created_datetime').html(moment(val['created_datetime']).format('DD/MM/YYYY HH:mm:ss'));
...
$.Edit = async function (citem) {
    ...
    $('#btn-save_exit').click(function (e) {
        ...
        var prodpurplan_stdmargin = $('#stockprice').find('#prodpurplan_stdmargin').val();
        ...
        if ($('#inventory1').find('#prodpurplan_grvat_exvat').is(':checked')) { var prodpurplan_grvat = 0; }
        else if ($('#inventory1').find('#prodpurplan_grvat_invat').is(':checked')) { var prodpurplan_grvat = 1; }
        else { var prodpurplan_grvat = 0; }
        ...
        $('#frm_data').parsley().on('form:submit', function () {
            ...
            if (authorize == 'ITM-MIS-01') {
                ...
                var data = {
                    ...
                    'prodpurplan_stdmargin': prodpurplan_stdmargin,
                    ...
                    //'prodpurplan_grvat': $('#inventory1').find('#prodpurplan_grvat_exvat').prop("checked") === true ? '1' : '2',
                    'prodpurplan_grvat': prodpurplan_grvat,
            ...
            //console.log("authorize-2", authorize);
            ...
            if (authorize == 'ITM-MIS-01' || authorize == 'ITM-PIT-01' || authorize == 'ITM-PIT-02' || authorize == 'ITM-PM-01' || authorize == 'ITM-PM-02' || authorize == 'ITM-PUR-01' || authorize == 'ITM-PUR-02' || authorize == 'ITM-PM-03') {
                ...
                fetch(itemmaster_update, {
                    ...
                    //console.log("itemmaster_update-1");
                    ...
                    toastr.success('Save Successfully!', function () {
                        ...
                        //oTable.destroy();
                        //$.List();
                        //console.log("itemmaster_update-2");
                    ...
                    setTimeout(function () {
                        //console.log("itemmaster_update-3");
                        //                    $.LoadingOverlay("hide");
                        $('.btn-save_form').prop('disabled', false);
                        $("#frm_data").parsley().reset();
                        $('#modal-frm_data').modal('hide');
                        //$.LoadingOverlay("hide");
                        setTimeout(function () {
                            //console.log("itemmaster_update-5");
                            $.LoadingOverlay("hide");
                            setTimeout(function () {
                                //console.log("itemmaster_update-6");
                                $.LoadingOverlay("hide");
                            }, 2500);
                        }, 1800);
                    }, 1200);
