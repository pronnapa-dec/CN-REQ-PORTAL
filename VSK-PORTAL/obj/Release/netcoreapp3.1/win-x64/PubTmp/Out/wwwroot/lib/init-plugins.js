...
$.gcode_init = async function () {

    let objGcode = {};

    let url = "http://192.168.1.247/intranet/acc-api/v1/gcode_get";

    await fetch(url + '?ctype=gcode_a').then(function (response) {
        return response.json();
    }).then(function (result) {
        let citem = [];
        citem.push(result.data)
        objGcode['c1'] = citem;
    });

    await localStorage.setItem("objGcode", JSON.stringify(objGcode));
...
$.MRP_MROUND = function (number, roundto){
    return roundto * Math.round(number / roundto);
}

$.MRP_MIN = function (numbers){

}

$.MRP_MAX = function (numbers) {

}

$.driver_get = function (driver_id) {
    ...
    fetch('https://vsk.tms-vcargo.com/api/tms/public/v1/driver/search', {
        ...
        if (result.statusCode === 200) {
            ...
            $.each(result.object, function (key, val) {
                ...
                driver_list.push({ id: val['code'], text: val['firstNameTh'] + ' ' + val['lastNameTh'] });

            });

            localStorage.setItem("objTRPDriver", JSON.stringify(driver_list));
