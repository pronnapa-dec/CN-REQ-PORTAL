...
let purpod_get = 'http://localhost:49705/api/PR_POE_IV_GR_bySale';

let export_poe = 'http://localhost:49705/PurExport/Pur_Poe_Export';

let update_pod = 'http://localhost:49705/api/PK_PO_IV_bySale_Update';

let customepr_url = 'http://localhost:49705/api/Custome_PR_Get';

/*
let purpod_get = 'http://192.168.1.247/intranet/pur-api/api/PR_POE_IV_GR_bySale';

let export_poe = 'http://192.168.1.247/intranet/pur-api/PurExport/Pur_Poe_Export';

let update_pod = 'http://192.168.1.247/intranet/pur-api/api/PK_PO_IV_bySale_Update';

let customepr_url = 'http://192.168.1.247/intranet/pur-api/api/Custome_PR_Get';
*/
        ...
        if (result.status === 'Error') {
            ...
            oTable = $('#tbl-list').DataTable({
                ...
                columns: [
                    ...
                    {
                        title: "<span style='font-size:11px;'>potra_startdate</span>",
                        data: "potra_startdate",
                        class: "tx-center",
                        visible: false,
                        render: function (data, type, row, meta) {
                            if (moment(data).format('DD/MM/YYYY') === '01/01/0001') {
                    ...
                    $.contextMenu({
                        ...
                        callback: async function (key, options) {
                            ...
                            let citem = {
                                ...
                                potra_startdate: data['potra_startdate']
                            };
...
$.Details = async function (citem) {
    ...
    fetch(customepr_url + '?pono_e=' + citem['PO_no']).then(function (response) {
        return response.json();
    }).then(function (result) {

        $.each(result.data, function (key, val) {
            $('#frm_data').find('#Sale_user').val(val['Sale_user']);
        });
